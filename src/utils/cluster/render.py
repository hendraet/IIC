...
from src.utils.cluster.cluster_eval import _get_assignment_data_matches, _clustering_get_data
...
def save_progress(config, net, mapping_assignment_dataloader, mapping_test_dataloader, index, sobel, render_count):
  ...
  """
    ...
    reordered_soft_preds = torch.zeros((num_samples, config.gt_k), dtype=soft_preds.dtype).cuda()
    for pred_i, target_i in match:
        reordered_soft_preds[:, GT_TO_ORDER[target_i]] += soft_preds[:, pred_i]  # 1-1 for IIC
    ...
    best_colours = [list((np.array(hsv_to_rgb(hue, 0.8, 0.8)) * 255.).astype(np.uint8)) for hue in hues]
    ...
    for colour_i, colours in enumerate(all_colours):
        ...
        image = np.ones((2 * (scale + border), 2 * (scale + border), 3), dtype=np.uint8) * 255
        ...
        for i in xrange(num_samples):
            ...
            image[pt_start[0]:pt_end[0], pt_start[1]:pt_end[1], :] = np.reshape(colour, (1, 1, 3))

        # add on the average image per cluster in the border
        # -------------------------
        # dataloaders not shuffled, or jittered here
        averaged_imgs = [np.zeros((config.input_sz[0], config.input_sz[1], 1)) for _ in xrange(config.gt_k)]
        ...
        for b_i, batch in enumerate(mapping_test_dataloader):
            ...
            for offset in xrange(n):
                ...
                averaged_imgs[predicted_cluster_render] += predicted_cluster_gt_weight * img
                averaged_imgs_norm[predicted_cluster_render] += predicted_cluster_gt_weight
        ...
        for c in xrange(config.gt_k):
            ...
            coord = (coord * (scale + half_border) + (scale + half_border)).astype(np.int32)
        ...
        img.save(os.path.join(prog_out_dir, "%d_run_%d_colour_%d_pointcloud_%s.png" %

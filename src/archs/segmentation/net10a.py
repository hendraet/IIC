...
class SegmentationNet10aTrunk(VGGTrunk):
    def __init__(self, config, cfg):
        ...
        assert (config.input_sz[0] % 2 == 0 and config.input_sz[1] % 2 == 0)
...
class SegmentationNet10aHead(nn.Module):
    def __init__(self, config, output_k, cfg):
        ...
        self.heads = nn.ModuleList([nn.Sequential(
            nn.Conv2d(num_features, output_k, kernel_size=1, stride=1, dilation=1, padding=1, bias=False),
            nn.Softmax2d()) for _ in xrange(self.num_subheads)
        ])
...
class SegmentationNet10a(VGGNet):
    cfg = [(64, 1), (128, 1), ('M', None), (256, 1), (256, 1), (512, 2), (512, 2)]  # 30x30 recep field
    ...
    def __init__(self, config):
        ...
        self.head = SegmentationNet10aHead(config, output_k=config.output_ks[0], cfg=SegmentationNet10a.cfg)

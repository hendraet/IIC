...
def main():
    ...
    reloaded_config_path = os.path.join(config.out_root, str(config.old_model_ind), "best_config.pickle")
    ...
    test_data = TenCropAndFinish(test_data, input_sz=old_config.input_sz, include_rgb=old_config.include_rgb)
    ...
    if not config.restart:
        ...
        net_features.load_state_dict(torch.load(model_path, map_location=lambda storage, loc: storage))
    ...
    if config.restart:
        ...
        optimiser_states = torch.load(os.path.join(config.out_dir, "latest_optimiser.pytorch"))
    ...
    for e_i in xrange(start_epoch, config.num_epochs):
        ...
        if e_i in config.lr_schedule:
            print("e_i %d, multiplying lr for opt trunk and head by %f" % (e_i, config.lr_mult))
        ...
        for i, (imgs, targets) in enumerate(train_loader):
            ...
            if (i % 100 == 0) or (e_i == start_epoch):
                print("batch %d of %d, loss %f, time %s" % (i, num_batches, float(loss.data), datetime.now()))
                sys.stdout.flush()
        ...
        if is_best or (e_i % 10 == 0):
            ...
            if is_best:
                ...
                torch.save({"opt_head": opt_head.state_dict(), "opt_trunk": opt_trunk.state_dict()},
